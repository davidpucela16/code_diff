...
class solve_sys():
    ...
    def set_Dirichlet_tissue_SS_sparse(self):
        ...
        self.phi_total=np.concatenate((phiv, phit))
    ...
    def solve_sparse_SS(self, *string):
        sol=sp.sparse.linalg.spsolve(self.A, self.phi_total)

        plot_solution(sol, geom, 9999, self.Rv,self.K_m, string[0], lim0=self.lim0, limF=self.limF)
...
#   ASSEMBLY VESSEL MATRIX
def assembly(obj, vessel , tissue):
    K_m=obj.K_m
    c=0
    inc_r, inc_z=obj.inc_r, obj.inc_z
...
#         main[i,:]=0
#         main[i,i]=1
#     elif i>len(tissue.phi)-zlen:
#         main[i,:]=0
#         main[i,i]=1
# 
# Up=np.append(vessel.main, vessel.coupl, axis=1)
# Down=np.append(tissue.coupl, main, axis=1)
# A=np.append(Up, Down, axis=0)
# 
# #Set Dirichlet inlet BCs:
# vessel.phi[:]=0 #vector where the encryption for the boundary conditions were written 
